import { addserviceTypeApi, addservicesApi, getAllServiceApi, getsevicetypeApi } from "./constrants";
  ...
  async () => {
    try {
      ...
      if (response.data.status === "success") {
        ...
        console.log("Error response in get services type Api => ", response.data);
        const err =
          response?.data?.message ||
          response?.message ||
          "Something went wrong!";
